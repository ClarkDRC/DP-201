...
1. In the Azure Portal, on the **Deploy VM Scale Set with Python Bottle server & AutoScale** blade, specify the following settings and initiate the deployment:
   ...
   - Resource group: the name of a new resource group **dp200-LabRG**
...
1. At the **Cloud Shell** command prompt, type in the following command and press **Enter** to list all resource groups you created in this lab:
   ...
   ```
   az group list --query "[?starts_with(name,'dp200')].[name]" --output tsv
   ```
...
1. At the **Cloud Shell** command prompt, type in the following command and press **Enter** to delete the resource groups you created in this lab
   ...
   ```
   az group list --query "[?starts_with(name,'dp200')].[name]" --output tsv | %{az group delete -n $_ --no-wait -y}
   ```
